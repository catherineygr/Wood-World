...
abstract class Mannequin extends TransformGroup {
	...
	public Mannequin(float scalex, float scaley, TextureLoader tl) {
		...
		Alpha a = new Alpha();
		a.setLoopCount(-1);
		a.setIncreasingAlphaDuration(1000);
		a.setDecreasingAlphaDuration(1000);
		a.setMode(Alpha.INCREASING_ENABLE|Alpha.DECREASING_ENABLE);
		RotationInterpolator mri = new RotationInterpolator(
				a,
				maintg
			);
		mri.setMaximumAngle(0);
		mri.setMaximumAngle((float)(Math.PI/3.0f));
		
		
		mri.setSchedulingBounds(boundingSphere);
		mri.setEnable(true);
